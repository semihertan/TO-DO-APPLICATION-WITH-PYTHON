...
def add_user(username, password):
    ...
    try:
        ...
        print("KullanÄ±cÄ± adÄ± zaten mevcut.")
    conn.close()
...
# kullanÄ±cÄ± bilgileri kontrol fonk.
...
def remove_completed_tasks():
    ...
    # Tekrar etmeyen tamamlanmÄ±ÅŸ gÃ¶revleri sil
    cursor.execute("DELETE FROM tasks WHERE status = 1 AND repeat_interval IS NULL")

    # Tekrar eden gÃ¶revlerin iÅŸaretlerini kaldÄ±r (tamamlanma durumunu sÄ±fÄ±rla)
...
def remove_past_due_tasks(user_id):
    ...
    # Tarihi geÃ§miÅŸ gÃ¶revleri seÃ§
    ...
    # Bildirimde gÃ¶stermek iÃ§in tarihi geÃ§miÅŸ gÃ¶revler listesi
    ...
    for task_id, task, due_date, repeat_interval in removed_past_due_tasks:
        if repeat_interval:  # EÄŸer gÃ¶rev tekrarlanÄ±yorsa
            due_date_dt = datetime.strptime(due_date, '%d.%m.%Y')
            next_due_date = None
            ...
            # DiÄŸer repeat interval'lar iÃ§in de eklenebilir...

            if next_due_date:
                # GÃ¶revi yeni tarihle tekrar ekle
                cursor.execute(
        ...
        # Orijinal gÃ¶revi sil
        cursor.execute("DELETE FROM tasks WHERE id = ?", (task_id,))

    conn.commit()
    conn.close()

    # Bildirimde gÃ¶stermek iÃ§in geri dÃ¶n
